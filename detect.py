    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                final_dict.append({"image_name": p.stem, 'bbox_list': []})
                # Write results
                for *xyxy, conf, cls in reversed(det):
                    if True:  # save_json
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # 原图像的xywh ,输出一个list格式。
                        # conf应该是图像的bbox_score  cls是一个序列数，names[int(cls)]是序列所对应的种类
                    #print('%.2f' % conf)
                    final_dict[-1]["bbox_list"].append(
                        {
                            "bbox_type": names[int(cls)],
                            "bbox_xmin": xywh[0]-0.5*xywh[2],
                            "bbox_ymin": xywh[1]-0.5*xywh[3],
                            "bbox_width": xywh[2],
                            "bbox_height": xywh[3],
                            "bbox_score": '%.2f' % conf
                        })
                    if save_txt:  # Write to file
                        tryandtry = xyxy2xywh(torch.tensor(xyxy).view(1, 4))
                        # print(tryandtry)
...
def main(opt):
    ...
    run(**vars(opt))
    pred_json = str('./predictions.json')  # 按照比赛要求导出的数据
